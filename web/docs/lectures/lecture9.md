...
1. Download the [led project](lecture9/img/led.tar) that blinks a led without timer. Check that the project compiles and blinks the led . write a function `void toggle_LED()` that uses a global variable `LEDstate` which correspond to the current status of the LED. (In general, it is very common in embedded system to have a *local copy* of the state of peripheral, just to know in which state we are). 
