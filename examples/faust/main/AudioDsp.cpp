...
fRunning(false)
{
  ...
  // initializing faust DSP
  fDSP = new mydsp();
  fDSP->init(fSampleRate);
  // MapUI is used to easily control the Faust parameters
  fUI = new MapUI();
  // connecting MapUI to Faust DSP
...
bool AudioDsp::start()
{
  if (!fRunning) {
    fRunning = true;
    fUI->setParamValue("gate",1); // setting gate to 1 to hear sound
...
void AudioDsp::audioTask()
{
  ...
  while (fRunning) {
    ...
    // calling Faust audio callback
    fDSP->compute(fBufferSize,NULL,outputs);
    
    // copying result into output buffer
    for (int channel = 0; channel < 2; ++channel){
